...
void runTest( int argc, char** argv) 
{
    ...
    int *itemsets,/*, *itemsets,*/ *referrence;
...
/*
    ...
    itemsets = (int *)malloc( max_rows * max_cols * sizeof(int) );
*/
	cudaMallocManaged((void **)&referrence,  max_rows * max_cols * sizeof(int) );	
	cudaMallocManaged((void **)&itemsets,  max_rows * max_cols * sizeof(int) );	
...
/*
	...
	cudaMemcpy(matrix_cuda, itemsets, sizeof(int) * size, cudaMemcpyHostToDevice);
*/
    dim3 dimGrid;
	...
	for( int i = block_width - 1  ; i >= 1 ; i--){
		dimGrid.x = i;
		dimGrid.y = 1;
		needle_cuda_shared_2<<<dimGrid, dimBlock>>>(referrence, itemsets//referrence_cuda, matrix_cuda
		                                      ,max_cols, penalty, i, block_width); 
	}


	cudaDeviceSynchronize();
    //cudaMemcpy(itemsets, matrix_cuda, sizeof(int) * size, cudaMemcpyDeviceToHost);
